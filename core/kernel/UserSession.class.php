...
final class UserSession extends DBWorker {
    ...
    public function read($phpSessId) {
        ...
        $res = $this->dbh->select(
            ...
            array('session_native_id' => addslashes($this->phpSessId))
        );
        ...
        // если такого сеанса в БД не существует
            ...
            try {
                ...
                $this->id = $this->dbh->modifyRequest(
                    "INSERT INTO {$this->tableName} (session_native_id, session_created, session_user_agent, session_ip) VALUES(%s, NOW(), %s, INET_ATON(%s))",
                    addslashes($this->phpSessId), $this->userAgent, E()->getRequest()->getClientIP()
                );
            }
            catch (Exception $e){
