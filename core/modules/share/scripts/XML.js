 ...
 *
 * @author https://github.com/kamicane/mootools-xml
...
var XML = {
            ...
            case 'element':
                ...
                for (var j = 0; j < attributesLength; j++) {
                    ...
                    if (attribute.nodeValue && attribute.nodeValue !=
                            'inherit') attributesHash[attribute.nodeName] =
                            attribute.nodeValue;
                }
    ...
    hashesToTree: function(hash) {
        var tree = [];
        for (var i = 0, l = hash.length; i <
                l; i++) tree.push(XML.hashToElement(hash[i]));
        return tree;
    },
    ...
    hashToElement: function(hash, tag) {
        ...
        else element = document.newElement(tag ||
                hash.tag, hash.attributes).adopt(XML.hashesToTree(hash.children));
        return element;
    },
    ...
    hashToHTML: function(hash, level) {
        ...
        var attributes = [''];
        for (var p in hash.attributes) attributes.push(p + '="' +
                hash.attributes[p] + '"');
        attributes = attributes.join(' ');
        var open = tabs + '<' + hash.tag + attributes + '>';
        var close = tabs + '</' + hash.tag + '>';
    ...
    hashesToHTML: function(hashes, level) {
        var html = [];
        for (var i = 0, l = hashes.length; i <
                l; i++) html.push(XML.hashToHTML(hashes[i], level));
        return html.join('\n');
    },
