...
//		      if (empty($feature->getValue())) continue;//modbysd remove empty values, fixes unrelated values in GOODs_List (carusel)
	...
	public function getFilterWhereConditions() {
		...
		if ( $target_ids = $this->getParam( 'id' ) ) {
			$result['goods_id'] =
				sprintf( "({$table_name}.goods_id in (%s))",
					( is_array( $target_ids ) ? implode( ',', $target_ids ) : $target_ids ) );
		} else {
			...
			if ( $this->dbh->tableExists( 'shop_goods_additional_categories' ) ) {
				...
				$result = [
					'smap_id' => sprintf(
						'(smap_id IN (%s) or goods_id in (%s))', implode( ',', $documentIDs ),
						implode( ',', $additional_goods ) ),
				];
			} else {
			...
			if ( $filter_data ) {
				...
				if ( isset( $filter_data['producers'] ) && ! empty( $filter_data['producers'] ) ) {
					...
					$result['producers'] = sprintf( '(producer_id IN (%s))',
						implode( ',', $filter_data['producers'] ) );
				}
	...
	protected function main() {
		parent::main();
		if ( $this->pager ) {
			$this->pager->setProperty( 'additional_url',
				substr( array_reduce( $this->getSortData(), function ( $p, $c ) {
					return $p . $c . '-';
				}, 'sort-' ), 0, - 1 ) . '/' );
		}
		// attachments in list
