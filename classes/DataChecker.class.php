    ...
    class DataChecker extends Model {
        ...
        private function checkDB() {
            ...
            if (!isset($this->dataset['host'])) {
                $this->dataset['host'] = $this->Config->database->host;
                $this->dataset['DBName'] = $this->Config->database->name;
                $this->dataset['username'] = $this->Config->database->username;
            }
            ...
            if($link) {
                ...
                if ($test_table[0]['Engine'] != 'InnoDB') {
                    ...
                    $this->getViewer()->addBlock('База данных поддерживает InnoDB!',Viewer::TPL_CHECKER_CONFIRM);
                }
        ...
        public static function isEnergineInstalled(){
            return file_exists('../.htaccess') && (filesize('../.htaccess') !== 0);
        }
