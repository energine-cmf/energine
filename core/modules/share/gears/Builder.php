	...
	protected function createField(
		...
		if ( in_array( $fieldInfo->getType(), [ FieldDescription::FIELD_TYPE_FILE ] ) ) {
			if (
				...
				$quick_upload_pid     = $this->dbh->getScalar( 'SELECT upl_id FROM share_uploads WHERE upl_path=%s LIMIT 1',
					$quick_upload_path );
			...
			if ( $fieldValue ) {
				...
				try {
					if ( $info = E()->FileRepoInfo->analyze( $fieldValue ) ) {
						...
						$playlist = [ ];
						$base     = pathinfo( $fieldValue, PATHINFO_DIRNAME ) . '/' . pathinfo( $fieldValue,
								PATHINFO_FILENAME );
			...
			if ( ! empty( $fieldValue ) || ( $fieldValue === 0 ) || ( $fieldValue === '0' ) ) {
				switch ( $fieldInfo->getType() ) {
					case FieldDescription::FIELD_TYPE_STRING:
						$fieldValue = E()->Utils->formatDate( $fieldValue,
							$fieldInfo->getPropertyValue( 'outputFormat' ), $fieldInfo->getType() );
						break;
